...
// Data structure to set the simulation parameters
struct Parameters {
	unsigned N_trials;
	unsigned L;
	double M;
...
void concurrent_measure(const Parameters& param) {
	...
	double p;
	unsigned i, N_extinct;

	std::osyncstream(std::cout) << "Starting simulations for M = " << param.M << "...\n";
	N_extinct = 0;
	for (i = 0; i < param.N_trials; i++) {
		sim.randomize();
		sim.set_t(0);
	...
	p = ((double) N_extinct)/param.N_trials;
	std::osyncstream(*param.out) << param.M << '\t' << p << '\t' << std::sqrt(p*(1-p)/param.N_trials) << '\n';

}

int main() {
	const unsigned L = 50;
	const unsigned N_trials = 1000;
